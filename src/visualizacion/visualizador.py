...
class Visualizador:
    ...
    def graficar_series_temporales(self, contaminantes: list[str] | None = None) -> go.Figure | None:
        ...
        if not disponibles:
            print("[series_temporales] ⚠️  Ningún contaminante solicitado está disponible todavía.")
            return None
    ...
    def graficar_pm25_vs_meteorologia(self) -> go.Figure | None:
        ...
        if not disponibles:
            print("[pm25_vs_meteorologia] ⚠️  No hay variables meteorológicas disponibles todavía.")
            return None
    ...
    def graficar_heatmap_correlaciones(self, columnas: list[str] | None = None) -> go.Figure | None:
        ...
        if len(disponibles) < 2:
            print("[heatmap] ⚠️  Se necesitan al menos 2 columnas numéricas disponibles para el heatmap.")
            return None
    ...
    def mapa_zonas_criticas(self, metrica: str = "pm25", agregacion: str = "mean") -> go.Figure | None:
        ...
        if not tiene_zona and not tiene_coordenadas:
            print("[mapa_zonas_criticas] ⚠️  Aún no hay columnas de ubicación ('zona' o 'latitud'/'longitud').")
            return None
        ...
        print(
            "[mapa_zonas_criticas] ℹ️  No hay coordenadas todavía; se muestra un ranking por "
